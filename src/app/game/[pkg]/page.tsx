...
async function getGameDetails(pkg: string): Promise<GameDetails | null> {
  try {
    ...
    console.error(`Error fetching game details for pkg: ${pkg}:`, error);
    return null;
  }
}
...
export default async function GameDetailPage({ params }: { params: { pkg: string } }) {
  const game = await getGameDetails(params.pkg);
